...
export function buildSchedulerWorkflow({ postgresCredentialId, whatsappCredentialId, includeWebhook = false }) {
  ...
  where c.status = 'scheduled'
    ...
    and exists (
      select 1 from public.wa_marketing_recipients pending
      where pending.campaign_id = c.id and pending.send_status = 'queued'
    )
...
return [{ json: { sql: "with selected as (select * from public.wa_marketing_select_recipients('" + id + "'::uuid)) select s.recipient_id, s.phone_e164, c.id as campaign_id, c.template_name, c.template_language, c.template_components from selected s join public.wa_marketing_campaigns c on c.id = '" + id + "'::uuid;" } }];`;
  ...
  const auditCode = `const recipients = $('Final eligibility check').all();
